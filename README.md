...
*   **Rich previews:** `yazi` includes support for image previews, video thumbnails, archive contents, and more, provided you have the necessary optional dependencies installed (which the `install.sh` script handles automatically). The official `video` preset plugin is tracked in this repo so video previews arrive with the normal Yazi install.
...
The Mero Terminal keymap keeps `Ctrl+Space` as the standard Herdr and tmux prefix: `Ctrl+Space`, then `d` detaches, `t` creates a tab, `,` renames the tab, `/` splits vertically, `-` splits horizontally, `p`/`n` cycle tabs, and arrows move pane focus. It also adds direct, seamless `Ctrl+Alt` shortcuts in both multiplexers: `c` creates a tab, `,` renames it, `/` and `-` split, `p`/`n` cycle tabs, and `h/j/k/l` focus panes. `Shift+Tab` is intentionally not used because ArchMerOS/Hyprland reserves it for window cycling. The installer also installs Herdr's Codex and Pi integrations when those harness directories exist, and installs the official global Herdr agent skill through `npx skills` or a direct raw-file fallback on lean machines.
